...
export async function POST(req: NextRequest) {
  const { trainerId, studentName, phone, email, packageName, price, address, city, pincode } = await req.json();
  ...
  // Save booking — use "PENDING" status (exists in your schema for both flows)
  const booking = await prisma.booking.create({
    data: {
      ...
      amount: price,
      status: "PENDING",   // works for both verified and unverified
...
function buildAdminCoordinationMessage(d: {
  bookingId: string; studentName: string; studentPhone: string; studentEmail?: string;
  trainerName: string; trainerPhone: string; trainerAddress: string;
  trainerWebsite: string; trainerGMB: string; packageName: string; price: number; city: string;
}) {
...
4. Pitch LearnDrive — 1st booking already done!
...
function buildAdminEmail(d: {
  ...
  <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;">
    ...
    <div style="background:#edf7f1;border-radius:12px;padding:20px;margin-bottom:24px;">
      ...
      <p style="margin:4px 0;"><strong>Amount:</strong> <span style="color:#e8821a;font-size:18px;font-weight:bold;">₹${d.price.toLocaleString("en-IN")}</span></p>
    </div>
    <div style="border:2px solid #e8821a;border-radius:12px;padding:20px;">
      <h3 style="margin:0 0 12px;color:#e8821a;">📋 Action Steps</h3>
      <ol style="margin:0;padding-left:20px;line-height:1.8;">
        ...
        <li>Pitch LearnDrive — 1st booking already done, ₹500 commission waived for joining!</li>
