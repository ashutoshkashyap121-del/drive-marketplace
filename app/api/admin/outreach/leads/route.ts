...
export default function AIOperationsPage() {
  ...
  useEffect(() => {
    ...
    setSecret(s);
    const h = { "Content-Type": "application/json", "x-admin-secret": s };
    fetchLeads(h);
    fetchPending(h);
  }, []);

  const fetchLeads = async (h?: Record<string, string>) => {
    try {
      const res = await fetch("/api/admin/outreach/leads", { headers: h || headers });
      if (res.ok) setLeads(await res.json());
    } catch {}
  };

  const fetchPending = async (h?: Record<string, string>) => {
    try {
      const res = await fetch("/api/admin/trainers/pending", { headers: h || headers });
