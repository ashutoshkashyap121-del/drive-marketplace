...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://learndrive.in/driving-school-fees-in-${city}`,
      siteName: "LearnDrive",
      locale: "en_IN",
      type: "website",
    },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "28px 24px" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E2E8F0", overflow: "hidden", marginBottom: 24 }}>
              ...
              {schoolsWithPrices.map((t, i) => (
                <div key={t.id} style={{
                  ...
                  padding: "16px 20px",
                  gap: 8,
                  borderBottom: i < schoolsWithPrices.length - 1 ? "1px solid #F1F5F9" : "none",
                  alignItems: "center",
                  ...
                  <Link href={`/trainers/${t.id}`} style={{ background: "#F59E0B", color: "#0F172A", textDecoration: "none", padding: "8px 14px", borderRadius: 8, fontWeight: 700, fontSize: 12 }}>
                    Book →
                  </Link>
                </div>
              ))}
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E2E8F0", padding: "28px 32px", marginBottom: 24 }}>
            ...
            <p style={{ color: "#555", fontSize: 14, lineHeight: 1.9, marginBottom: 12 }}>
              In {meta.display}, DL packages usually range from ₹4,500 to ₹8,000 depending on the trainer, location, and hours. Most schools include simulator/hatch session, city road practice, test track run, and RTO preparation.
            </p>
            <p style={{ color: "#555", fontSize: 14, lineHeight: 1.9 }}>
              LearnDrive shows verified schools with transparent pricing so you can book the package that fits your budget without any hidden fees.
            </p>
          </div>

          <div style={{ marginBottom: 24 }}>
            <h3 style={{ fontFamily: "'Sora',sans-serif", fontSize: 15, color: "#555", marginBottom: 12 }}>Fees in other cities</h3>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {Object.entries(CITY_META)
                ...
                .map(([slug, m]) => (
                  <Link
                    key={slug}
                    href={`/driving-school-fees-in-${slug}`}
                    style={{
                      fontSize: 13,
                      background: "#fff",
