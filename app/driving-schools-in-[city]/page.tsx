...
// ── Tell Next.js exactly which city slugs to build ───────────────────────────
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title,
    description,
    keywords: [`driving school in ${meta.display}`, `driving classes ${meta.display}`, `learn driving ${meta.display}`, `driving licence ${meta.display}`, `motor training school ${meta.display}`].join(", "),
...
export default async function CityLandingPage({ params }: Props) {
  ...
  // If slug doesn't match a known city, 404
  if (!meta) notFound();

  const trainers = await prisma.trainer.findMany({
    where: { city: { equals: meta.display, mode: "insensitive" }, status: "APPROVED" },
    orderBy: [{ rating: "desc" }, { createdAt: "desc" }],
    select: { id: true, name: true, city: true, experience: true, rating: true, basePrice: true, packagesJson: true, adminNotes: true, languages: true },
  });
      ...
      <main style={{ minHeight: "100vh", background: "#F8F7F4", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 24px" }}>
          ...
          <div style={{ marginTop: 40, background: "#fff", borderRadius: 20, border: "1px solid #E2E8F0", padding: "28px 32px" }}>
            <h2 style={{ fontFamily: "'Sora', sans-serif", fontSize: 18, color: "#0F172A", marginBottom: 12 }}>How to Get a Driving Licence in {meta.display}</h2>
