...
function requestHeaders(includeCsrf = false): HeadersInit {
  return includeCsrf
    ? {
...
export default function AIOperationsPage() {
  ...
  const sendToLead = async (lead: Lead) => {
    ...
    addLog(`${lead.name}: WhatsApp=${data.waSuccess ? "OK" : "FAIL"} SMS=${data.smsSuccess ? "OK" : "FAIL"}`);
    fetchLeads();
  };
  ...
  const reviewAll = async () => {
    ...
    const data = await res.json();
    addLog(`AI done: approved ${data.approved}, rejected ${data.rejected}, flagged ${data.flagged}`);
    setResults(data.details || []);
    setLoading(false);
    fetchPending();
  };

  const pendingLeads = leads.filter((lead) => lead.outreachStatus === "PENDING");
  const contactedLeads = leads.filter((lead) => lead.outreachStatus === "CONTACTED");
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "white", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 20px" }}>
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ color: "#64748b", fontSize: 14 }}>Automated trainer sourcing, outreach and approval</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 32 }}>
          {[
            ...
            <div
              key={stat.label}
              style={{ background: "rgba(255,255,255,0.05)", borderRadius: 12, padding: 20, border: "1px solid rgba(255,255,255,0.08)" }}
            >
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
          {[["outreach", "Outreach"], ["approve", "AI Approval"]].map(([key, label]) => (
            <button
              key={key}
              onClick={() => setTab(key as "outreach" | "approve")}
              style={{
                padding: "10px 24px",
                borderRadius: 10,
                border: "none",
                ...
                fontWeight: 600,
                fontSize: 14,
                background: tab === key ? "#f59e0b" : "rgba(255,255,255,0.07)",
          ...
          <div>
            {tab === "outreach" && (
              <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.08)", overflow: "hidden" }}>
                <div style={{ padding: "20px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <button
                    onClick={sendToAll}
                    disabled={loading || pendingLeads.length === 0}
                ...
                {leads.length === 0 ? (
                  <div style={{ padding: 40, textAlign: "center", color: "#475569" }}>
                    <p style={{ fontSize: 32, marginBottom: 12 }}>No leads</p>
            ...
            {tab === "approve" && (
              <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.08)", overflow: "hidden" }}>
                ...
                {pending.length === 0 ? (
                  <div style={{ padding: 40, textAlign: "center", color: "#475569" }}>
                    <p style={{ fontSize: 32, marginBottom: 12 }}>Done</p>
                    <p>No pending applications right now.</p>
                  </div>
                ) : (
                  <div>
                    {pending.map((trainer) => (
                      <div key={trainer.id} style={{ padding: "16px 24px", borderBottom: "1px solid rgba(255,255,255,0.04)", display: "flex", alignItems: "flex-start", gap: 16 }}>
                        <div style={{ flex: 1 }}>
                          <p style={{ fontWeight: 600, marginBottom: 2 }}>{trainer.name}</p>
                          <p style={{ fontSize: 13, color: "#64748b" }}>{trainer.mobile} - {trainer.city} - {trainer.experience} yrs exp</p>
                          <p style={{ fontSize: 12, color: "#475569", marginTop: 2 }}>{(trainer.vehicleTypes || []).join(", ")}</p>
                          {trainer.adminNotes && <p style={{ fontSize: 11, color: "#7c3aed", marginTop: 4 }}>{trainer.adminNotes}</p>}
                        </div>
                        <button
                          onClick={() => reviewOne(trainer)}
                ...
                {results.length > 0 && (
                  <div style={{ padding: "20px 24px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    {results.map((result, index) => (
                      <div key={index} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                        <div>
                          <p style={{ fontWeight: 600, fontSize: 14 }}>{result.name} <span style={{ color: "#64748b", fontWeight: 400 }}>score: {result.score}/100</span></p>
                          <p style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>{result.reason}</p>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
            ...
            <div style={{ padding: 16, height: 500, overflowY: "auto" }}>
              {log.length === 0 ? (
                ...
                log.map((entry, index) => (
                  <p key={index} style={{ fontSize: 12, color: "#64748b", marginBottom: 8, lineHeight: 1.5, borderBottom: "1px solid rgba(255,255,255,0.03)", paddingBottom: 8 }}>
                    {entry}
                  </p>
                ))
              )}
