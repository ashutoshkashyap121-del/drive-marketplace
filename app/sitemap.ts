...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages: MetadataRoute.Sitemap = [
    ...
    { url: `${BASE}/rc-check`,                  priority: 0.9, changeFrequency: "monthly", lastModified: now },
    { url: `${BASE}/challan-check`,             priority: 0.9, changeFrequency: "monthly", lastModified: now },
  ...
  // ── Locality pages — from generated JSON ─────────────────────────────────
  let localityPages: MetadataRoute.Sitemap = [];
  try {
    const localityData = JSON.parse(
      fs.readFileSync(path.join(process.cwd(), "public", "locality-data.json"), "utf-8")
    );
    localityPages = Object.keys(localityData).map((slug) => ({
      url:             `${BASE}/driving-schools-in-${slug}`,
      priority:        0.7,
      changeFrequency: "weekly" as const,
      lastModified:    now,
    }));
  } catch {
    // locality-data.json not yet generated — skip silently
  }

  return [...staticPages, ...feePages, ...cityPages, ...blogPages, ...trainerPages, ...localityPages];
}
